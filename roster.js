...
                <div class="bg-gradient-to-r from-\[#14171d] to-\[#0a0e14] border border-outline-variant/20 border-dashed rounded-\[24px] p-8 md:p-10 flex flex-col md:flex-row items-center justify-between gap-6 w-full group hover:border-primary/50 transition-colors cursor-pointer" onclick="window.openCreateSquadModal()">
...
                <div class="bg-\[#14171d] rounded-3xl p-10 border border-outline-variant/10 shadow-lg flex flex-col items-center justify-center text-center w-full col-span-full">
...
            if (!squad) {
                html += `<div class="hidden md:block pointer-events-none"></div>`;
                return;
            }
...
            const orderClass = rank === 1 ? 'order-1 md:order-2 md:scale-105 z-20 md:-translate-y-6 min-h-[260px] md:h-[340px]' :
                               rank === 2 ? 'order-2 md:order-1 min-h-[260px] md:h-[300px]' :
                                            'order-3 md:order-3 min-h-[260px] md:h-[280px]';
 
            const borderStyle = rank === 1 ? 'border-[#FFD700]/50 shadow-[0_0_30px_rgba(255,215,0,0.15)]' :
                                rank === 2 ? 'border-[#C0C0C0]' :
                                             'border-[#CD7F32]';
 
            const bgClass = rank === 1 ? 'bg-gradient-to-t from-[#1a1d24] to-[#0a0e14]' : 'bg-gradient-to-t from-[#14171d] to-[#0a0e14]';
 
            let badgeHtml = '';
            if (rank === 1) badgeHtml = `<div class="absolute -top-4 md:-top-5 bg-\[#FFD700] text-\[#0a0e14] px-4 py-1.5 rounded-full font-black flex items-center justify-center text-\[10px] md:text-xs shadow-\[0\_0\_20px\_rgba(255,215,0,0.5)] tracking-widest z-20 whitespace-nowrap border border-\[#FFD700]/50">👑 RANK 1</div>`;
            else if (rank === 2) badgeHtml = `<div class="absolute -top-4 md:-top-5 bg-\[#C0C0C0] text-\[#0a0e14] px-4 py-1.5 rounded-full font-black flex items-center justify-center text-\[10px] md:text-xs shadow-lg z-20 whitespace-nowrap">🥈 RANK 2</div>`;
            else if (rank === 3) badgeHtml = `<div class="absolute -top-4 md:-top-5 bg-\[#CD7F32] text-white px-4 py-1.5 rounded-full font-black flex items-center justify-center text-\[10px] md:text-xs shadow-lg z-20 whitespace-nowrap">🥉 RANK 3</div>`;

            html += `
                <div class="w-\[85vw] md:w-full shrink-0 snap-center ${orderClass} rounded-\[28px] md:rounded-\[32px] ${bgClass} border ${borderStyle} ${podiumClass} relative flex flex-col items-center justify-center p-6 md:p-8 cursor-pointer group hover:scale-\[1.02] transition-transform" onclick="window.location.href='squad-details.html?id=${squad.id}'">
 
                    ${badgeHtml}
                    <div class="w-24 h-24 md:w-32 md:h-32 absolute -top-12 md:-top-16 rounded-\[24px] bg-surface-container border-\[4px] border-\[#0a0e14] overflow-hidden shadow-xl z-10 group-hover:border-primary/50 transition-colors">
...
                    <div class="mt-8 md:mt-12 w-full text-center flex flex-col items-center flex-1 justify-between">
                        <div class="flex flex-col items-center justify-center flex-1 w-full">
                            <h3 class="font-headline font-black italic uppercase text-white leading-tight ${isFirstPlace ? 'text-2xl md:text-3xl' : 'text-xl md:text-2xl'} mb-1 group-hover:text-primary transition-colors">
                                <span class="text-outline-variant/70 text-\[10px] md:text-xs">[${safeAbbr}]</span> <br/>${safeName}
                            </h3>
                            <p class="text-\[11px] md:text-xs text-outline-variant font-bold uppercase tracking-widest mb-4 mt-2">${wins}W - ${losses}L</p>
                        </div>
 
                        <div class="bg-\[#0a0e14]/50 border border-outline-variant/10 rounded-2xl px-6 py-3 w-full mt-auto">
                            <p class="text-\[9px] md:text-\[10px] text-outline font-bold uppercase tracking-widest mb-1">Rating</p>
...
        myProfileContainer.innerHTML = `
            <div class="bg-gradient-to-r from-\[#14171d] to-\[#0a0e14] rounded-\[24px] p-6 border border-tertiary/40 shadow-\[0\_4px\_30px\_rgba(202,165,255,0.1)] hover:brightness-110 transition-all cursor-pointer flex flex-col md:flex-row items-center justify-between gap-6 group" onclick="window.location.href='profile.html?id=${myData.id}'">
 
                <div class="flex items-center gap-5 w-full md:w-auto">
...
 
            let badgeHtml = '';
            if (rank === 1) badgeHtml = `<div class="absolute -top-4 md:-top-5 bg-\[#FFD700] text-\[#0a0e14] px-4 py-1.5 rounded-full font-black flex items-center justify-center text-\[10px] md:text-xs shadow-\[0\_0\_20px\_rgba(255,215,0,0.5)] tracking-widest z-20 whitespace-nowrap border border-\[#FFD700]/50">👑 MVP</div>`;
            else if (rank === 2) badgeHtml = `<div class="absolute -top-4 md:-top-5 bg-\[#C0C0C0] text-\[#0a0e14] px-4 py-1.5 rounded-full font-black flex items-center justify-center text-\[10px] md:text-xs shadow-lg z-20 whitespace-nowrap">🥈 RANK 2</div>`;
            else if (rank === 3) badgeHtml = `<div class="absolute -top-4 md:-top-5 bg-\[#CD7F32] text-white px-4 py-1.5 rounded-full font-black flex items-center justify-center text-\[10px] md:text-xs shadow-lg z-20 whitespace-nowrap">🥉 RANK 3</div>`;

            html += `
                <div class="w-\[85vw] md:w-full shrink-0 snap-center ${orderClass} rounded-\[28px] md:rounded-\[32px] ${bgClass} border ${borderStyle} ${podiumClass} relative flex flex-col items-center justify-center p-6 md:p-8 cursor-pointer group hover:scale-\[1.02] transition-transform" onclick="window.location.href='profile.html?id=${player.id}'">
 
                    ${badgeHtml}
                    <div class="absolute w-24 h-24 md:w-32 md:h-32 -top-12 md:-top-16 rounded-full border-\[4px] border-\[#0a0e14] bg-surface-container overflow-hidden shadow-xl z-10 group-hover:border-primary/50 transition-colors">
...
                    <div class="mt-8 md:mt-12 w-full text-center flex flex-col items-center flex-1 justify-between">
                        <div class="flex flex-col items-center justify-center flex-1 w-full">
                            <div class="flex items-center justify-center gap-1.5 mb-2 h-5">
...
                                ${safeName}
                            </h3>
                            <p class="text-\[11px] md:text-xs text-outline-variant font-bold uppercase tracking-widest mt-1 mb-4">${fullPos}</p>
                        </div>
 
                        <div class="bg-\[#0a0e14]/50 border border-outline-variant/10 rounded-2xl px-6 py-3 w-full mt-auto">
                            <p class="text-\[9px] md:text-\[10px] text-outline font-bold uppercase tracking-widest mb-1">Score</p>
...
                    submitBtn.innerHTML = `<span>Create Squad</span><span class="material-symbols-outlined text-\[18px]">add\_task</span>`;
