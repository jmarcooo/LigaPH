...
                borderStyle = 'border-t-4 border-[#FFD700] shadow-lg shadow-[#FFD700]/10';
            } else if (rank === 2) {
                badgeColor = 'bg-[#C0C0C0] text-[#0a0e14] shadow-md';
                borderStyle = 'border-t-4 border-[#C0C0C0] shadow-sm';
            } else {
                badgeColor = 'bg-[#CD7F32] text-white shadow-md';
                borderStyle = 'border-t-4 border-[#CD7F32] shadow-sm';
            }

            html += `
                <div class="w-\[85vw] sm:w-\[300px] md:w-full shrink-0 snap-center bg-gradient-to-t from-\[#14171d] to-\[#1a1d24] rounded-\[24px] md:rounded-\[32px] border border-outline-variant/10 ${borderStyle} flex flex-col items-center justify-center p-6 md:p-8 cursor-pointer group hover:-translate-y-1 transition-transform relative mt-4" onclick="window.location.href='squad-details.html?id=${squad.id}'">
 
...
                    <div class="w-24 h-24 md:w-28 md:h-28 mt-2 rounded-2xl bg-surface-container border border-outline-variant/20 overflow-hidden shadow-lg mb-6 group-hover:scale-105 transition-transform">
                        <img src="${logoUrl}" onerror="this.onerror=null; this.src='${getFallbackLogo(safeName)}';" class="w-full h-full object-cover">
                    </div>
...
            html += `
                <div class="w-\[85vw] sm:w-\[300px] md:w-full shrink-0 snap-center bg-gradient-to-t from-\[#14171d] to-\[#1a1d24] rounded-\[24px] md:rounded-\[32px] border border-outline-variant/10 ${borderStyle} flex flex-col items-center justify-center p-8 cursor-pointer group hover:-translate-y-1 transition-transform relative mt-4" onclick="window.location.href='profile.html?id=${player.id}'">
 
                    <div class="absolute -top-3 ${badgeColor} px-4 py-1.5 rounded-full font-black flex items-center justify-center text-\[10px] uppercase tracking-widest z-20 whitespace-nowrap">
                        ${rank === 1 ? '👑 MVP' : rank === 2 ? '🥈 RANK 2' : '🥉 RANK 3'}
                    </div>

                    <div class="w-24 h-24 md:w-28 md:h-28 mt-2 rounded-full border-\[4px] border-\[#0a0e14] bg-surface-container overflow-hidden shadow-lg mb-6 group-hover:scale-105 transition-transform">
                        <img src="${photoUrl}" onerror="this.onerror=null; this.src='${getFallbackAvatar(safeName)}';" class="w-full h-full object-cover">
                    </div>
...
                            ${safeName}
                        </h3>
                        <p class="text-\[10px] md:text-\[11px] text-outline-variant font-bold uppercase tracking-widest mb-6">${fullPos}</p>
 
                        <div class="bg-\[#0a0e14]/50 border border-outline-variant/10 rounded-xl px-4 py-2.5 w-full">
