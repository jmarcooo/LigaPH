    ...
    filterBtns.forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            filterBtns.forEach(b => {
                b.classList.remove('active', 'text-[#ff751f]', 'border-b-[#ff751f]');
                b.classList.add('text-gray-500', 'border-transparent');
            });
            const target = e.target;
            target.classList.add('active', 'text-[#ff751f]', 'border-b-[#ff751f]');
...
async function fetchNews() {
    try {
        ...
        fullNewsContainer.innerHTML = `
            <div class="text-center py-12 bg-red-50 dark:bg-red-500/10 rounded-3xl border border-red-200 dark:border-red-500/20">
                <span class="material-symbols-outlined text-red-500 text-5xl mb-3">error</span>
                <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-1">Failed to Connect to Database</h3>
                <p class="text-sm text-gray-500 dark:text-gray-400 px-4">${error.message}</p>
            </div>
        `;
    } finally {
...
function renderFeed() {
    ...
    if (currentFilter !== 'all') {
        // Handle filter variations gracefully
        filtered = allArticles.filter(a => a.category?.toLowerCase().includes(currentFilter.toLowerCase().replace('s', '')));
    }
    ...
    if (filtered.length === 0) {
        ...
        heroNewsContainer.classList.add('hidden');
        fullNewsContainer.innerHTML = `
            <div class="text-center py-12 bg-white dark:bg-[#14171d] rounded-3xl border border-gray-200 dark:border-white/10 shadow-sm">
                <span class="material-symbols-outlined text-gray-300 dark:text-gray-600 text-5xl mb-3">newspaper</span>
                <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-1">No News Articles Found</h3>
                <p class="text-sm text-gray-500">There are no articles currently published in the database.</p>
            </div>
        `;
        return;
    }
        ...
        <div onclick="openArticle('${hero.id}')" class="group cursor-pointer bg-white dark:bg-[#14171d] rounded-3xl overflow-hidden border border-gray-200 dark:border-white/10 shadow-sm hover:border-[#ff751f]/50 transition-all duration-300">
            <div class="relative aspect-video w-full overflow-hidden bg-gray-200 dark:bg-white/5">
                ...
                <div class="absolute bottom-0 left-0 p-6 md:p-8 w-full">
                    ...
                    <h2 class="font-headline font-black italic text-2xl md:text-4xl text-white uppercase tracking-tight leading-tight mb-2 drop-shadow-md group-hover:text-[#ff751f] transition-colors">${hero.title || 'Untitled Article'}</h2>
        ...
        fullNewsContainer.innerHTML = list.map(article => `
            <div onclick="openArticle('${article.id}')" class="flex flex-col md:flex-row gap-6 group border-b border-gray-200 dark:border-white/10 pb-8 cursor-pointer">
                ...
                <div class="flex-1 flex flex-col justify-center">
                    ...
                    <h3 class="font-headline font-black text-xl md:text-2xl italic uppercase tracking-tighter text-gray-900 dark:text-white group-hover:text-[#ff751f] transition-colors leading-tight mb-3">${article.title || 'Untitled Article'}</h3>
...
window.openArticle = function(id) {
    ...
    // Clean content format for Drop Cap
...
function getExcerpt(content, maxLength = 150) {
    if (!content) return 'Read the full story to learn more...';
