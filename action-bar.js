...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const desktopBtn = document.getElementById('desktop-search-btn'); // If added to desktop header
        ...
        if (currentFilter === 'all' || currentFilter === 'squads') {
            ...
            if (matchedSquads.length > 0) {
                ...
                matchedSquads.forEach(s => {
                    ...
                    resultsHtml += `
                        <div onclick="window.location.href='squad-details.html?id=${squadId}'" class="flex items-center gap-4 p-3 bg-surface-container-highest hover:bg-surface-bright rounded-xl border border-outline-variant/10 cursor-pointer transition-colors group mb-2">
    ...
    // Desktop Keyboard Shortcut (Cmd+K or Ctrl+K)
